...
def med_reshape(image, new_shape):
    ...
    # Populate the reshaped_image array (padded with 0-values where we don't have dimensions)
    # with values from the original image
    reshaped_image[0:image.shape[0],
                   0:image.shape[1],
                   0:image.shape[2]] = image
