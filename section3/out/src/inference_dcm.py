...
def get_predicted_volumes(pred):
    ...
    # <YOUR CODE HERE>
    # There are two lables that correspond to the presence of hippocampal volume
    # We want them as binary for each pixel, and then sum
    # Note that there are variable names specified here -- use 'em
    # See comment for ant and pos designations
    
    # TODO do we need to apply a unit scaler to this? 
    # For now, going to assume equivalency between pixels are units.
    volume_ant = np.sum(np.where(pred == 1, 1, 0))
    volume_post = np.sum(np.where(pred == 2, 1, 0))
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                        ...
                        f"Anterior volume: {str(inference['anterior'])}\n"
                        f"Posterior volume: {str(inference['posterior'])}\n"
                        f"Total volume: {str(inference['total'])}\n",
...
#     pimg.paste(pil_i, box=(10, 280))
    ...
    # I could be missing something, but I think we need to draw this text on each of our slices for it
    # to be visible in the report
    
    # To do THIS, we need to subset the original volume into slices, and render over each
#     for slice_idx in slice_nums:
        
#         # Pull the slice
#         slice = orig_vol[slice_idx]
        
#         # Now we're going to use some of the lines that were suggested...
#         nd_img = np.flip((slice/np.max(slice))*0xff).T.astype(np.uint8)
#         pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize(400, 400)
#         pimg.paste(pil_i, box=(10, 280))
...
def get_series_for_inference(path):
    ...
    # <YOUR CODE HERE>
    # Hold series that we have identified as relevant
    series_for_inference = []
    
    for dicom_file in dicoms:       
        # What is the information we need to identify the files we need?
        
        # I've seen reference "HippoCrop" being available as infor in the SeriesDescription, but I'm not 
        # seeing that this field _registers_ this information...
  
        # SOLUTION: this was a total pain
        # So here's the thing: within the code, there is a assumption that the hippocampal studies will be the 
        # MOST RECENT study within the directory. This is not accurate. Update this, seems to run. 
        if dicom_file.SeriesDescription == "HippoCrop":
            series_for_inference.append(dicom_file)
        #else:
            #print(dicom_file.SeriesDescription)
        
        # Issues with where this goes next...
        #series_for_inference = np.array(temp_series)
...
#     study_dir = sorted(subdirs, key=lambda dir: os.stat(dir).st_mtime, reverse=True)[0]
    # UPDATE Note that in my case, the most recent directory is NOT a hippocampal series. Adjust accordingly
    ...
    # FIXME What is the information that will be moved? I'm missing a title...
    # DONE check the contents of `send_volume.sh`. Unclear on where was documented, but fine.
    # localhost == 127.0.0.1
