...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # First, let's get our 3D data to have a closer look (assign to a new variable)
        # Note that we also have to recast some datatypes 
        slice_data = self.data[slc[0]]
        sample["image"] = torch.from_numpy(slice_data["image"][slc[1]][None, :]).type(torch.cuda.FloatTensor)
        sample["seg"] = torch.from_numpy(slice_data["seg"][slc[1]][None, :]).type(torch.cuda.LongTensor)
